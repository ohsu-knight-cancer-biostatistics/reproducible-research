...
- fig.height & fig.width -- height/width of plot (default is 7 in)
- dev -- graphical device (figure file format) that image will be saved as
- fig.cap -- set figure's caption/title
- fig.lb -- set figure's label for cross-references (LaTeX)
- out.height & out.width -- height/width to scale the figure in presentation doc 
...
```
Alternately, this _scatterplot_ can be created by sourcing a code file hosted on GitHub
...
``` {r echo=FALSE, label=WB_plot, fig.cap="Caption is placed here", fig.height=6, fig.width=6, fig.align='center'}
pairs(x = No_tail, main="WB development indicators", lower.panel=NULL)
```
...
```{r label=wickham_book, fig.height=3, fig.width=4, fig.align='center'}
...
```{r label=time_plot, eval=TRUE, echo=FALSE, warning=FALSE, fig.align='center'}
...
**Data source:** Online via Univ. of Washington professor Ken Rice's [webpage](http://faculty.washington.edu/kenrice/heartgraphs/nhaneslarge.csv)
